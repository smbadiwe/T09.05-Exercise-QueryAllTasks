...
public class TaskContentProvider extends ContentProvider {
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        int match = sUriMatcher.match(uri);
        Cursor cursor; // URI to be returned
        ...
        switch (match) {
            case TASKS:
                ...
                if ( cursor != null ) {
                    cursor.setNotificationUri(getContext().getContentResolver(), TaskContract.TaskEntry.CONTENT_URI);
                } else {
                    throw new android.database.SQLException("Failed to process query. " + uri);
                }
                break;
            // Set the value for the returnedUri and write the default case for unknown URI's
            case TASK_WITH_ID:
                String id = uri.getPathSegments().get(1);
                cursor = db.query(TABLE_NAME, projection, "_ID = " + id, selectionArgs, null, null, sortOrder);
                if ( cursor != null ) {
                    cursor.setNotificationUri(getContext().getContentResolver(), TaskContract.TaskEntry.CONTENT_URI);
                } else {
                    throw new android.database.SQLException("Failed to process query " + uri);
